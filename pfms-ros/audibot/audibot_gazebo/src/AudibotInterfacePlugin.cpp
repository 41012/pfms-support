...
namespace audibot_gazebo {
                          ...
                          EventManager &/*_eventMgr*/)
    {
        ...
        if (!this->model_.Valid(_ecm)) {
            ignerr << "Audibot plugin should be attached to a model entity\n";
            return;
        }
    ...
    void AudibotInterfacePlugin::recvGearCmd(const gz::msgs::UInt32& msg) {
        switch (msg.data()) {
            ...
            break;
            default:
            ignwarn << "Gear shift ignored: unsupported gear enum " << (int)msg.data() << " (0 = DRIVE, 1 = REVERSE)" << std::endl;
            break;
        }
    }
    ...
    void AudibotInterfacePlugin::PostUpdate(const UpdateInfo& _info, const EntityComponentManager &_ecm) {
        ...
        // Detect teleportation: if position change is too large, reset velocity to prevent instability
        double position_change = sqrt(dx * dx + dy * dy);
        double yvel, yawvel;
        if (position_change > 0.5 || dt <= 0.0) {  // Teleported or invalid dt
            this->current_speed_ = 0.0;
            yvel = 0.0;
            yawvel = 0.0;
        } else {
            this->current_speed_ = (dx * cosf(yaw) + dy * sinf(yaw)) / dt;
            yvel = (-dx * sinf(yaw) + dy * cosf(yaw)) / dt;
            yawvel = dyaw / dt;
        }
        this->last_vehicle_pose_ = vehicle_pose;
...
// Register plugin
IGNITION_ADD_PLUGIN(audibot_gazebo::AudibotInterfacePlugin,
              System,
...
IGNITION_ADD_PLUGIN_ALIAS(audibot_gazebo::AudibotInterfacePlugin, "ignition::gazebo::systems::AudibotInterfacePlugin")